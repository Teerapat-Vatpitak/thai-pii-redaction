...
- **#2 Harden localhost API — เสร็จ (2026-07-16, PR #37)**: CORS strict allowlist + TrustedHost + session TTL (เดิม) และ boot token แล้ว — `AIGUARD_TOKEN` env gate `POST /api/shutdown` + `DELETE /api/session/{id}` ผ่าน `X-AIGuard-Token` (`compare_digest`) grace path เมื่อไม่ตั้ง token; `/api/health` มี `capabilities.token_required`; data plane รอ token channel ของ extension (native messaging #16) โดยตั้งใจ
...
- **#4 Collision-safe pseudonym + span merge — เสร็จ (2026-07-16)**: `anonymize` เช็ค uniqueness ก่อนเขียน vault (re-roll seed ผ่าน `attempt=` สูงสุด 8 ครั้ง + suffix `#N` เป็น last resort เหมือน web path) และ `SessionVault.write` raise เมื่อ pseudonym ชนกับ original คนละคน; pipeline CLI ใช้ `aggregate.dedupe_spans` ตัด FP/TB span overlap ก่อน replace (module กลางเดียวกับ web/benchmark)
- **#5 Single-source version + OSS front door — เสร็จ (2026-07-16, PR #36)**: `VERSION` ไฟล์เดียว + `scripts/bump_version.py` + `scripts/check_version.py` (fail-safe drift gate) ใน CI + ROADMAP.md/CONTRIBUTING.md/SECURITY.md/CHANGELOG.md — เหลือ action ฝั่ง owner: เปิด Private vulnerability reporting ใน repo settings ตาม note ใน SECURITY.md
- **#6 CWS submission — ฝั่ง repo เสร็จ (2026-07-16, PR #38)**: `_locales` th/en + manifest i18n, privacy policy (ทุก claim ตรวจกับโค้ดจริง), permission justification, listing copy + screenshot checklist, `scripts/package_extension.py` — เหลือ action ฝั่ง owner: สมัคร CWS dev account ($5), ถ่าย screenshot, อัปโหลด zip + กรอกฟอร์ม
- **#7 booth latency — ปิดเป็น moot**: งาน poster จบไปแล้ว (10 ก.ค.) ตาราง latency ใน booth-checklist ไม่มีผู้ใช้แล้ว
