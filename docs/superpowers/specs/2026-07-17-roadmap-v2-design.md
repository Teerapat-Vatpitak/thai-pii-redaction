...
2. **Audit รอบใหม่ทั้ง repo** — multi-agent, ครอบ `pii_redactor/`, `app/`, `extension/`, `desktop/`, `scripts/`, CI/release workflows เขียนเอกสาร findings ลง `docs/superpowers/specs/` เป็น artifact ถาวร (ทุก finding มีสถานะ + หลักฐาน) แล้วปิด findings ตามความรุนแรง — งานนี้ต้องจบก่อน tag
3. **Housekeeping** —
