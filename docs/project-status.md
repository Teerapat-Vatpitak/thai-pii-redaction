...
| Desktop app | Acceptance pending | The isolated rehearsal shell and packaged sidecar launched successfully. Actual Tauri UI checks passed token/surrogate restore, PDPA analysis, PDF upload/preview/download, theme, PII-free audit view, and updater check. The run exposed `[object Object]` PDF labels, a missing PDPA-PDF download action, stale site copy, and a colon-delimited house-number redaction gap; current source fixes all four with regressions. A fresh build must verify those fixes plus hotkeys and close/reopen before release; CI updater signing remains separate. |
