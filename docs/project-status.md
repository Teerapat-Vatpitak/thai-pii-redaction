...
| Browser extension | Acceptance pending | Versioned ZIP packaging and 52 JS/DOM tests pass. Current-site mask/restore still requires loading the exact candidate in Chrome and running the real-site checklist. |
...
| Microsoft 365 Add-in | Acceptance pending | TypeScript task pane, HTTPS localhost proxy, Word/Excel/PowerPoint adapters, stale-selection guards, memory-only session state, atomic writeback checks, safe error disclosure, manifest validation, and Node 22 CI lane are implemented. All 59 Office tests pass. Separate Microsoft-validated local XML transports exist for all three hosts, while the release manifest still exposes Word only; Excel/PowerPoint stay hidden until promotion gates close. The unified package registered but was not acquired by the current Office tenant/client. Local XML real-host evidence now covers Word token/surrogate exact restore and mixed size/color/highlight fail-closed; Excel text-only Apply/Restore, byte-for-byte formula preservation, and stale-range cancellation; and PowerPoint selected-text Apply/Restore plus mixed/no-selection fail-closed. Earlier Word evidence also covers health/offline, Detect, PDPA Analyze, stale-selection cancellation, and protected Pathumma preview with an unused-token warning. Table/multi-paragraph and failure scenarios, Excel/PowerPoint Ask AI, deeper PowerPoint isolation/capability fallback, and unified acquisition remain open. XML schema/results do not satisfy the unified promotion gate, and no host is fully accepted. Current product version remains 2.4.0 until the full storefront gate closes. |
