...
| Microsoft 365 Add-in | Verified locally | TypeScript task pane, HTTPS localhost proxy, Word/Excel/PowerPoint adapters, stale-selection guards, memory-only session state, atomic writeback checks, safe error disclosure, manifest validation, and Node 22 CI lane are implemented. All 59 Office tests pass. Synthetic-PII local XML real-host acceptance passed for Word, Excel, and PowerPoint: Word Preview/Apply/Restore, table/multiple-paragraph Copy-only, failure handling, and protected Pathumma response insertion; Excel text-only Apply/Restore with byte-for-byte formula preservation and stale-range cancellation; and PowerPoint selected-text Apply/Restore plus isolation/capability fail-closed cases. The Word-only unified manifest acquires and opens its task pane, passes authoritative schema validation, and packages deterministically. Custom ribbon activation on the packaged distribution transport has not been independently confirmed, so this is not a Marketplace or broad Office-distribution claim. |
