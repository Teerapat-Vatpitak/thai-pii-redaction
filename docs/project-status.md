...
| Version/tag/release pipeline | Acceptance pending | v2.4.0 is published and a `2.4.1` patch release candidate is in preparation for the compatible manifest/release fixes. GitHub reports no configured `main` protection; the release PR, tag workflow, checksums, attestations, and install smoke must still pass before publication. Full Office/storefront support remains gated for a later minor release. |
