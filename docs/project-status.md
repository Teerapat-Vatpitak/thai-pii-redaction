...
| Blind evaluation set | Verified | `blind-v1` frozen 2026-07-28: 185 documents / 479 entities across the 11 types plus a 52-document negative slice, authored and reviewed in isolated contexts and committed only as an authenticated blob + lock (counts pinned in `benchmark/data/blind-v1.lock.json`). Scoring is aggregate-only under a 6-reveal budget with a hash-chained committed audit log. Reveals used: 3 of 6 — the freeze baseline; the close of accuracy campaign 1 (PRs #90-#91), which generalized (overall F2 0.837 to 0.903, non-overlapping bootstrap CIs); and the close of campaign 2 (PR #93, NAME), which did NOT generalize — gold NAME recall rose 0.774 to 0.905 but blind overall F2 stayed flat (0.903 to 0.898, overlapping CIs) and blind NAME precision fell 0.748 to 0.700. Recorded as-is; the campaign is kept for its gold-register gains and the follow-up direction is engine-level NER work, not more cues. Protocol: [2026-07-28 ADR](decisions/2026-07-28-blind-set-protocol.md). |
