...
owner-approved ADR; authoritative PDF source-to-box intervals remain a
separate Phase 8 unit.

Phase 8 second-unit status:
**shared provider orchestration implemented in current source; external
...
response/mapping evidence; shared provider orchestration has no fresh live or
packaged/real-host evidence; and PDF boxes are selected through global
