...
| Blind evaluation set | Verified | `blind-v1` frozen 2026-07-28: 185 documents / 479 entities across the 11 types plus a 52-document negative slice, authored and reviewed in isolated contexts and committed only as an authenticated blob + lock (counts pinned in `benchmark/data/blind-v1.lock.json`). Scoring is aggregate-only under a 6-reveal budget with a hash-chained committed audit log. Reveals used: 2 of 6 — the freeze baseline, and the close of accuracy campaign 1 (PRs #90-#91), which confirmed the gold-tuned gains generalize (overall F2 0.837 to 0.903 with non-overlapping bootstrap CIs, recall unchanged). Protocol: [2026-07-28 ADR](decisions/2026-07-28-blind-set-protocol.md). |
