...
| Desktop app | Acceptance pending | An isolated Windows build rehearsal produced and smoked the NSIS payload, release shell, and frozen sidecar. The candidate later launched with a healthy packaged sidecar, and token/surrogate restore, report, and PDF API paths passed against that running process. Windows automation could not activate the Tauri window, so interactive installed-app UI/update rehearsal and CI updater signing remain. |
