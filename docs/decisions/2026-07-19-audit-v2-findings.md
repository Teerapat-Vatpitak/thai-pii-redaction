...
| VAULT-5 | `output_validator.py:139-144` | truncation heuristic flag ข้อความ >20 ตัวที่จบด้วยเลข/อักษรละติน → `halt=True` → exporter raise ExportError กับ output ปกติ (reproduce: จบด้วยเบอร์โทรที่ restore แล้ว) path web รอดเพราะ `_NOISY_PREFIXES` filter | FIXED | invert เป็น blocklist ตัวอักษร/เลขทุก script + ตัวปิด (วงเล็บ/เครื่องหมายคำพูด) เป็น ending ที่ valid เหลือ flag เฉพาะ ending กลางประโยคจริง (จบด้วย comma/hyphen/วงเล็บเปิด) trade-off ที่รับไว้ การตัดที่พอดีขอบตัวอักษร/เลขตรวจไม่ได้ |
...
| id | ไฟล์ | อาการย่อ | สถานะ | แนวทางแก้ |
|---|---|---|---|---|
| EXT-2 | `content.js:95-115` + `sites.js:57-90` | doMask จับ composer ก่อน await แล้วเขียนกลับหลัง response `writeComposer` คืน true เสมอ ไม่ตรวจว่าเขียนลง composer จริง รายงานสำเร็จทั้งที่ text ดิบยังอยู่ (fails-open, false positive) | FIXED | doMask re-query composer หลังเขียนแล้วเทียบ readComposer กับ sanitized (whitespace-insensitive) ไม่ตรง = maskFailed ไม่ขึ้นสำเร็จ side panel ตรวจแล้วไม่เข้าข่าย (paste-mode ไม่เขียน composer) |
| EXT-3 | `content.js:107-116` | Mask ล้มเหลวแจ้งแค่ status ตัวเล็กมุมจอ ไม่บล็อก/ดัก submit ปุ่ม Send เว็บ เส้นทางส่ง text ดิบเปิดเสมอ (fails-open เชิงสถาปัตยกรรม) | FIXED | ทุกทาง mask ล้มขึ้น overlay เต็มจอ (backdrop บังหน้าเว็บจนกดปิดเอง = submit block ชั่วคราวโดยพฤตินัย) ไม่ดัก Enter/submit ตรง ๆ เพราะ selector ปุ่ม Send ต่อ site เปราะและเสี่ยงพังการพิมพ์ปกติ |
| EXT-4 | `content.js:67-91,135` | ข้อความ restore (PII จริง) ถูกใส่ลง DOM ของเว็บ AI ผ่าน overlay ธรรมดา script ของเว็บ (session replay/analytics) อ่านได้ ขัด threat model (ยืนยันแล้วว่าไม่รั่วออกเครือข่ายอื่นนอก localhost เป็น DOM exposure) | FIXED | overlay ทั้งหมดย้ายเข้า closed shadow root (host เปล่าอยู่ในหน้าเว็บ `shadowRoot` = null ฝั่งเพจ traversal ไม่ถึงข้อความ) style ย้ายเข้า shadow ด้วย (`OVERLAY_CSS` ใน content.js) เพจ patch `attachShadow` ดักไม่ได้เพราะ content script อยู่ isolated world เทส pin ทั้งด้านซ่อนจากเพจและด้านแสดงให้ผู้ใช้เห็นจริง เพจยังลบ host node ได้ (ซ่อน UI ไม่ใช่อ่าน) side panel ที่เป็นตัวเลือกแรงกว่าไว้เป็น upgrade ภายหลังถ้าจำเป็น |

### Desktop

| id | ไฟล์ | อาการย่อ | สถานะ | แนวทางแก้ |
|---|---|---|---|---|
| DESK-2 | `sidecar.rs:48-56` + `api.js:1` | `spawn()` ถือว่าอะไรก็ตามที่ listen 127.0.0.1:8000 คือ backend ของเราโดยไม่ verify identity process ที่ squat port ก่อนได้รับ clipboard PII และ text/PDF ทั้งหมดผ่าน data plane ที่ไม่มี auth | FIXED | ตรวจ image name ของ process ที่ถือพอร์ต (netstat+tasklist / lsof, parser เป็น pure fn มีเทส) เจ้าของไม่ใช่ aiguard = shell ปฏิเสธ start ทั้งแอป (alert + exit, fail-closed) dev attach ต้องตั้ง `AIGUARD_ALLOW_ATTACH=1` เอง ขีดจำกัดที่บันทึก มาลแวร์ same-user ที่ตั้งชื่อ exe ว่า aiguard.exe ยังผ่าน (พ้น threat model) |
| DESK-3 | `capabilities/default.json:14-17` | webview ได้ `shell:allow-execute` args=true + clipboard read/write + global-shortcut ทั้งที่ frontend ไม่ใช้ (ทำ Rust-side หมด) XSS ใน webview จึง escalate ไป re-exec AIGuard.exe และอ่าน clipboard ได้ | FIXED | ตัด grant ทั้งสามกลุ่มออก เหลือ core/tray/updater — งาน shortcut/clipboard/sidecar ทำฝั่ง Rust ซึ่งไม่ผ่าน capability อยู่แล้ว `tests/test_desktop_capabilities.py` pin ไฟล์ไว้ |
| DESK-4 | `screen-report.js:63,99` | `b.count`, `r.direct_pii_count` interpolate ลง innerHTML โดยไม่ `escapeHtml` (ต่างจาก field อื่น) backend ที่ถูก squat (DESK-2) คืน string แทน number ทำ XSS ใน webview ที่มี IPC grant (DESK-3) | FIXED | ห่อ `Number(...) || 0` ทั้งสองจุด เทส jsdom ยิง payload markup ผ่าน analyze ที่ mock แล้ว pin ว่าไม่มี `<img` โผล่ใน DOM (`desktop/tests/`) |
| DESK-5 | `sidecar.rs:184-188` + `lib.rs:26-36` | reap sidecar เฉพาะ `ExitRequested` shell crash/kill (ไม่มี Windows Job Object) orphan backend และ token ตายไปกับ shell instance ใหม่ attach แต่ shutdown ไม่ได้ (token mismatch) | FIXED | ใช้ fallback kill ตามที่เสนอ shell ใหม่ verify ว่าเจ้าของพอร์ตคือ aiguard (single-instance ตัดกรณี shell คู่ = เป็น orphan แน่) แล้ว taskkill tree + spawn ใหม่พร้อม token สด ขีดจำกัดที่บันทึก orphan ยังมีชีวิตจนกว่า shell จะเปิดรอบถัดไป (Job Object `KILL_ON_JOB_CLOSE` เป็น backlog ถ้าอยาก instant-kill) |
