...
|---|---|
| [2026-08-05 HTTP contract v2 privacy boundary](2026-08-05-http-contract-v2.md) | The main HTTP API cuts directly to strict v2 response projections with no v1 fallback absent a real consumer; mapping-oriented fields and raw guard/Section 26 excerpts leave the wire, offsets are half-open Unicode code points, outbound-capable operations fail closed, health separates control/data-plane authority, and server plus first-party clients migrate atomically. Implementation remains pending; worker envelope v1 and the separate `aiguard-aift` service are unchanged |
| [2026-07-29 processing receipt (มาตรา 39)](2026-07-29-processing-receipt.md) | The PDPA section 39 record is a per-run slip, not a cumulative RoPA — a register would mean retaining exactly what the in-memory vault and the stateless hosted path promise not to — and it is authenticated by rerunning the input and comparing digests rather than by a signature, which would prove who issued a receipt and nothing about whether it is true. `entity_id` and `score` stay outside the digest; the verifier ships in the same version as the issuer |
