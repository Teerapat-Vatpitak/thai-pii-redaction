  ...
  $("#a-go").addEventListener("click", async () => {
    ...
    try {
      ...
      $("#a-out").innerHTML = `
        ...
        ${r.recommendations.length ? `<div class="card"><b>คำแนะนำ</b><ul>${r.recommendations.map((c) => `<li><b>[${c.level}]</b> ${escapeHtml(c.title)} <span class="muted">${escapeHtml(c.desc)}</span></li>`).join("")}</ul></div>` : ""}
      `;
    } catch (e) {
