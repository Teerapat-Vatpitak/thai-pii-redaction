...
class TestServerRestoreBoundary:
    ...
    def test_restore_defect_is_clean_500(self, client, monkeypatch):
        ...
        resp = client.post("/api/roundtrip", json={"text": "สวัสดีครับผมชื่อสมชาย", "provider": "fake"})
        assert resp.status_code == 500
        assert resp.json()["detail"] == "restore failed (RuntimeError)"
    ...
    def test_non_string_provider_return_is_502(self, client, monkeypatch):
        ...
        resp = client.post("/api/roundtrip", json={"text": "สวัสดีครับ", "provider": "fake"})
