...
| `pii_redactor/guard/injection.py` | ชั้นตรวจ prompt injection แบบ rule-based (แยกจาก PII detection และ leak_guard); `scan_injection` → `GuardFinding[]`; bypass corpus บันทึกช่องโหว่เป็น xfail ตรง ๆ |
...
- `POST /api/guard {text}` → `{guard[], flagged}` — ตรวจ prompt injection ภาษาไทย/อังกฤษ แบบ rule-based ชั้นแรก (`pii_redactor/guard/injection.py`) จับ 5 กลุ่ม (instruction_override / role_hijack / exfiltration / hidden_chars / suspicious_payload) **แจ้งเตือน ไม่บล็อก** ออกแบบให้เสียบ classifier ชั้นสองได้ ไม่อ้างว่ากันได้หมด `/api/sanitize` และ `/api/roundtrip` เพิ่มคีย์ `guard[]` แบบ additive (สแกนข้อความดิบก่อน clean เพราะ hidden char คือสัญญาณ)
